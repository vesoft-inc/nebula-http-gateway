...
func (valWrap ValueWrapper) AsTime() (*TimeWrapper, error) {
	if valWrap.value.IsSetTVal() {
		...
		time, err := GenTimeWrapper(rawTime)
		if err != nil {
			return nil, err
		}
		return time, nil
...
func (valWrap ValueWrapper) AsDateTime() (*DateTimeWrapper, error) {
	if valWrap.value.IsSetDtVal() {
		...
		timeDate, err := GenDateTimeWrapper(rawTimeDate)
		if err != nil {
			return nil, err
		}
		return timeDate, nil
...
func (valWrap ValueWrapper) AsPath() (*PathWrapper, error) {
	...
	path, err := GenPathWrapper(valWrap.value.GetPVal())
	if err != nil {
		return nil, err
	}
	return path, nil
...
func (valWrap ValueWrapper) String() string {
	...
	if value.IsSetNVal() {
		...
		dateTime, _ := GenDateTimeWrapper(rawDateTime)
		return fmt.Sprintf("%d-%02d-%02dT%02d:%02d:%02d.%06d",
			dateTime.dateTime.GetYear(),
			dateTime.dateTime.GetMonth(),
			dateTime.dateTime.GetDay(),
			dateTime.dateTime.GetHour(),
			dateTime.dateTime.GetMinute(),
			dateTime.dateTime.GetSec(),
			dateTime.dateTime.GetMicrosec())
