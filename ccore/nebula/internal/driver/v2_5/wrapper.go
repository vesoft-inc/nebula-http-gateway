...
func (b dateBuilder) Build() types.Date {
	return newDateWrapper(b.build())
}

func (b dateBuilder) build() *nthrift.Date {
	date := nthrift.NewDate()
	date.Year = b.date.GetYear()
	date.Month = b.date.GetMonth()
	date.Day = b.date.GetDay()
	return date
}
...
func (b timeBuilder) Build() types.Time {
	return newTimeWrapper(b.build())
}

func (b timeBuilder) build() *nthrift.Time {
	time := nthrift.NewTime()
	time.Hour = b.time.GetHour()
	time.Minute = b.time.GetMinute()
	time.Sec = b.time.GetSec()
	time.Microsec = b.time.GetMicrosec()
	return time
}
