...
import (
	...
	graph3_0 "github.com/vesoft-inc/nebula-http-gateway/ccore/nebula/internal/thrift/v3_0/graph"
...
type (
	defaultGraphClient struct {
		graph *graph3_0.GraphServiceClient
	}

	authResponse struct {
		*graph3_0.AuthResponse
	}

	executionResponse struct {
		*graph3_0.ExecutionResponse
	}
)
...
func newGraphClient(transport thrift.Transport, pf thrift.ProtocolFactory) types.GraphClientDriver {
	return &defaultGraphClient{
		graph: graph3_0.NewGraphServiceClientFactory(transport, pf),
	}
}
...
func (c *defaultGraphClient) VerifyClientVersion() (*types.VerifyClientVersionResp, error) {
	req := graph3_0.NewVerifyClientVersionReq()
